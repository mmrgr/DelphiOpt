...
def render(source: Path, output: Path) -> None:
    ...
    keys = ("agent_mix", "mode", "scheduler", "status", "speedup", "cost_usd", "total_tokens", "latency_seconds", "llm_calls", "benchmark_runs", "rounds", "proposal_diversity")
    ...
    def bars(metric: str, color: str) -> str:
        ...
        return "".join(f"<div class='bar' style='background:{color};width:{max(2, 420 * float(row.get(metric, 0)) / maximum):.1f}px'>{html.escape(str(row.get('agent_mix')))} / {html.escape(str(row.get('mode')))} / {html.escape(str(row.get('scheduler')))}: {float(row.get(metric, 0)):.3f}</div>" for row in rows)
    ...
    model_frequency = "".join(f"<li>{html.escape(model)}: {count}</li>" for model, count in sorted(model_counts.items())) or "<li>No model calls recorded</li>"
    expert_reputation = "".join(f"<div class='bar' style='background:#6a7d2f;width:{max(2, 420 * sum(values) / len(values)):.1f}px'>{html.escape(expert)}: {sum(values) / len(values):.3f}</div>" for expert, values in sorted(expert_values.items())) or "No observations"
    disagreement = "".join(f"<div><strong>{html.escape(str(row.get('agent_mix')))} / {html.escape(str(row.get('mode')))} / {html.escape(str(row.get('scheduler')))}</strong>: {', '.join(f'R{index + 1}={float(value):.3f}' for index, value in enumerate(row.get('round_disagreements', []))) or 'no rounds'}</div>" for row in rows)
