...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "optimize":
        ...
        optimize_summary = OptimizationRuntime(config).optimize(project)
        print(json.dumps(optimize_summary.to_dict(), indent=2))
        return 0 if optimize_summary.status in {"accepted", "rejected"} else 1
    if args.command == "benchmark":
        ...
        gate = CorrectnessGate(sandbox).run(project, str(cfg.get("test_command", "pytest -q")), float(config.get("benchmark", {}).get("timeout_seconds", 120)), lint_command=str(cfg["lint_command"]) if cfg.get("lint_command") else None, budget=budget)
        result = BenchmarkEngine(**{key: value for key, value in config.get("benchmark", {}).items() if key in {"warmups", "repetitions", "timeout_seconds"}}, sandbox=sandbox).run(str(cfg.get("benchmark_command", "python benchmark.py")), project, budget, gate.passed)
        print(json.dumps({"correctness": gate.passed, "gate_reason": gate.reason, "test_output": gate.test_result.stdout[-2000:], "benchmark": result.to_dict(), "budget": budget.snapshot()}, indent=2))
