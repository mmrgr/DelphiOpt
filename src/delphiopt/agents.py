...
class ExpertAgent:
    ...
    async def analyze(self, project_context: str, budget: BudgetManager, tracer: RunTracer, round_number: int, feedback: str = "", max_tokens: int = 900) -> Proposal | None:
        ...
        for attempt in range(2):
            ...
            try:
                ...
                response = await self.provider.generate(current_prompt, model=self.model_name, max_tokens=max_tokens, temperature=self.temperature)
                ...
                tracer.record("agent_error", round=round_number, expert=self.expert_id, model=self.model_name, attempt=attempt + 1, error=str(exc))
            ...
            tracer.record("agent_call", round=round_number, expert=self.expert_id, model=response.model, provider=response.provider, prompt_tokens=response.input_tokens, completion_tokens=response.output_tokens, cost=response.cost_usd, latency=response.latency_seconds, max_tokens=max_tokens, repair_attempt=attempt, proposal=proposal.to_dict())
            return proposal
        return None

    async def revise(self, project_context: str, feedback: str, budget: BudgetManager, tracer: RunTracer, round_number: int, max_tokens: int = 900) -> Proposal | None:
