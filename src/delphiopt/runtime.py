...
from .profiler import StaticProfiler
...
class OptimizationRuntime:
    ...
    def optimize(self, project: str | Path) -> RunSummary:
        ...
        limits = BudgetLimits(**{key: value for key, value in self.config.get("budget", {}).items() if key in BudgetLimits.__dataclass_fields__})
        ...
        engine = BenchmarkEngine(warmups=int(benchmark_cfg.get("warmups", 2)), repetitions=int(benchmark_cfg.get("repetitions", 5)), timeout_seconds=timeout, sandbox=sandbox)
        trace.record("config", config=self.config, python_version=__import__("sys").version, project=str(project_path), random_seed=self.config.get("seed", 0), git_commit=self._git_commit(project_path))
        ...
        trace.record("correctness", stage="baseline", passed=baseline_gate.passed, reason=baseline_gate.reason, stages=[{"command": item.command, "return_code": item.return_code, "timed_out": item.timed_out} for item in baseline_gate.stages], return_code=baseline_gate.test_result.return_code, stdout=baseline_gate.test_result.stdout[-3000:], stderr=baseline_gate.test_result.stderr[-3000:])
        ...
        if not baseline.samples_ms:
            summary = RunSummary(run_id, str(project_path), "baseline_failed", baseline.median_ms, baseline.median_ms, 1.0, False, 0, 0, stop_reason="baseline correctness or benchmark failed", cost_usd=budget.usage.cost_usd, total_tokens=budget.usage.total_tokens)
            return self._finish(trace, project_path, summary, budget)
        ...
        scheduler = make_scheduler(str(scheduler_cfg.get("strategy", "adaptive_voi")), reputation=reputation, base_tokens=int(scheduler_cfg.get("base_tokens", 900)), tool_budget=int(scheduler_cfg.get("tool_budget", 2)))
        protocol_cfg = self.config.get("delphi", {})
        weights = CandidateWeights(**{key: float(value) for key, value in protocol_cfg.get("candidate_weights", {}).items() if key in CandidateWeights.__dataclass_fields__})
        protocol = DelphiProtocol(reputation=reputation, max_rounds=int(protocol_cfg.get("max_rounds", 3)), disagreement_threshold=float(protocol_cfg.get("disagreement_threshold", 0.08)), minority_bonus=float(protocol_cfg.get("minority_bonus", 0.12)), weights=weights)
        convergence = ConvergencePolicy(protocol.disagreement_threshold, protocol.max_rounds, int(protocol_cfg.get("no_improvement_rounds", 2)))
        stopping = StoppingPolicy(float(protocol_cfg.get("marginal_gain_threshold", 0.02)), float(protocol_cfg.get("min_utility_per_usd", 0.1)))
        ...
        for round_number in range(1, protocol.max_rounds + 1):
            ...
            feedback = protocol.feedback(last_proposals, f"baseline median={baseline.median_ms:.3f} ms") if round_number > 1 and last_proposals else ""
            completed.clear()
            while len(completed) < len(experts):
                decision = scheduler.schedule(SchedulerState(list(experts), difficulty=min(1.0, len(context) / 5000), disagreement=protocol.disagreement(last_proposals) if last_proposals else 1.0, candidate_gain=max(0.1, (last_proposals[0].expected_speedup - 1) if last_proposals else 1.0), remaining_budget_ratio=budget.remaining_ratio(), completed_experts=completed))
                ...
                trace.record(
                    "scheduling_decision", round=round_number, decision=decision.to_dict(),
                ...
                for model_alias in model_candidates:
                    ...
                    if model_cfg:
                        call_agent = ExpertAgent(
                            expert_id, agent.persona, str(model_cfg.get("model", model_alias)),
            ...
            should_stop, policy_reason = stopping.evaluate(expected_speedup=candidates[0].expected_speedup if candidates else 1.0, expected_cost_usd=max((candidate.implementation_cost for candidate in candidates), default=0.0), remaining_budget_ratio=budget.remaining_ratio())
            if should_stop:
            ...
            for candidate in candidates[: min(3, len(candidates))]:
                ...
                try:
                    shutil.copytree(project_path, workspace / "project", dirs_exist_ok=True, ignore=shutil.ignore_patterns(".git", ".delphiopt", "__pycache__"))
                    ...
                    trace.record("patch", round=round_number, proposal=candidate.to_dict(), applied=patch.applied, files=patch.files, diff=patch.diff, explanation=patch.explanation)
                    ...
                    trace.record("correctness", stage="candidate", round=round_number, proposal_id=candidate.id, passed=gate.passed, reason=gate.reason, stages=[{"command": item.command, "return_code": item.return_code, "timed_out": item.timed_out} for item in gate.stages], return_code=gate.test_result.return_code, stdout=gate.test_result.stdout[-3000:], stderr=gate.test_result.stderr[-3000:])
                    if not gate.passed:
                        reputation.update(candidate, correctness=False, actual_speedup=1.0)
                        continue
                    result = engine.run(benchmark_command, candidate_project, budget, True)
                    speedup = baseline.median_ms / result.median_ms if result.median_ms else 0.0
                    conservative_speedup = baseline.ci95_low_ms / result.ci95_high_ms if result.ci95_high_ms and result.ci95_high_ms != float("inf") else 0.0
                    coefficient_of_variation = result.stddev_ms / result.mean_ms if result.mean_ms and result.mean_ms != float("inf") else float("inf")
                    trace.record("benchmark", stage="candidate", round=round_number, proposal_id=candidate.id, result=result.to_dict(), speedup=speedup, conservative_speedup=conservative_speedup, coefficient_of_variation=coefficient_of_variation)
                    ...
                    if result.samples_ms and speedup >= meaningful_speedup and conservative_speedup >= meaningful_speedup and coefficient_of_variation <= max_cv:
                        implementer.sync_accepted_files(candidate_project, project_path, patch.files)
                        best_ms, winning = result.median_ms, candidate.id
                        stop_reason = "verified improvement accepted"
                        trace.record("candidate_decision", proposal_id=candidate.id, decision="accepted", speedup=speedup, reputation=reputation.snapshot())
                        summary = RunSummary(run_id, str(project_path), "accepted", baseline.median_ms, best_ms, baseline.median_ms / best_ms, True, round_number, candidates_evaluated, winning, stop_reason, budget.usage.cost_usd, budget.usage.total_tokens, proposal_diversity)
                        ...
                        return self._finish(trace, project_path, summary, budget)
                    trace.record("candidate_decision", proposal_id=candidate.id, decision="rejected", speedup=speedup, reputation=reputation.snapshot())
                finally:
            ...
            should_stop, policy_reason = convergence.evaluate(round_number=round_number, disagreement=round_disagreement, ranking=ranking, previous_ranking=previous_ranking, stagnant_rounds=stagnant_rounds, remaining_budget_ratio=budget.remaining_ratio())
            if should_stop:
                stop_reason = policy_reason
                break
            previous_ranking = ranking
        summary = RunSummary(run_id, str(project_path), "rejected", baseline.median_ms, best_ms, baseline.median_ms / best_ms if best_ms else 1.0, True, len(protocol.rounds), candidates_evaluated, winning, stop_reason, budget.usage.cost_usd, budget.usage.total_tokens, proposal_diversity)
        self._attach_experiment_metrics(summary, reputation, round_disagreements, models_used)
        reputation.save(reputation_path)
        return self._finish(trace, project_path, summary, budget)
    ...
    def _finish(self, trace: RunTracer, project: Path, summary: RunSummary, budget: BudgetManager) -> RunSummary:
        ...
        (project / ".delphiopt" / "runs" / f"{summary.run_id}.summary.json").write_text(json.dumps(summary.to_dict(), indent=2), encoding="utf-8")
        return summary

    def _attach_experiment_metrics(self, summary: RunSummary, reputation: ExpertReputationManager, disagreements: list[float], models_used: dict[str, int]) -> None:
    ...
    def _experts(self) -> dict[str, ExpertAgent]:
        ...
        for expert_id, settings in self.config.get("experts", {}).items():
            ...
            result[expert_id] = ExpertAgent(expert_id, str(settings.get("persona", expert_id)), str(model_cfg.get("model", model_name)), provider_from_config(model_cfg))
        return result
...
def awaitable_run(agent: ExpertAgent, context: str, feedback: str, budget: BudgetManager, trace: RunTracer, round_number: int, max_tokens: int) -> Proposal | None:
    import asyncio

    coroutine = agent.revise(context, feedback, budget, trace, round_number, max_tokens) if feedback else agent.analyze(context, budget, trace, round_number, max_tokens=max_tokens)
