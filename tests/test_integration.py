...
from delphiopt.telemetry import read_events
...
def test_confirmation_can_keep_verified_patch_out_of_source(tmp_path: Path) -> None:
    source = Path(__file__).parents[1] / "examples" / "demo_project"
    project = tmp_path / "demo_project"
    shutil.copytree(source, project)
    (project / "benchmark.py").write_text(
    ...
    summary = OptimizationRuntime().optimize(project, confirm=lambda proposal, diff, speedup: False)
    assert summary.status == "accepted_dry_run"
    assert "wanted = set(wanted)" not in (project / "target.py").read_text(encoding="utf-8")
...
def test_interrupted_run_resumes_from_checkpoint(tmp_path: Path) -> None:
    ...
    first = OptimizationRuntime(config).optimize(project)
    checkpoint = project / ".delphiopt" / "runs" / f"{first.run_id}.checkpoint.json"
    assert checkpoint.exists()

    config["delphi"]["max_rounds"] = 2
    resumed = OptimizationRuntime(config).optimize(project, resume_run=first.run_id)
    events = read_events(checkpoint.parent, first.run_id)
